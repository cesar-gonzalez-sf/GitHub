--Text
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--CREATE Procedure [dbo].[zSAV_CI_OraEmailDocVta]
-------------------------------------------------------------------------      
...
declare    
   ...
   set @NRO_DOCUMENTO_PADRE = 10745674
   set @TIPO_DOCUMENTO_PADRE = 'BLV'
   set @Nro_Impreso = '0014283250'
 ...
 BEGIN TRY      
  ...
  ----------------------------------------------------------------------------------------------------------------------      
	...
	begin
		...
		Select @Productos = @Productos + '<p>
		<tr>
		<td>
			<b><img src="'+case IsNull(dat.url_Imagen_Principal,'0') when '0' then 'https://www.imperial.cl/img/no-image.jpg' else dat.url_Imagen_Principal+'&height=87&width=87'end+'" align="left" width=87 height=87>
			<br>
			<br>' + dat.DESC_PUBLICA + '
			</b>
			<br>
			<br>
			&nbsp;&nbsp;&nbsp;&nbsp;Fecha Comprometida ' + Convert(varchar,Age.FechaDespacho,103) + '
		</td>
		<td>$' + replace(REPLACE(CONVERT(VARCHAR,CONVERT(Money, age.Precio_Lista),1),'.00',''),',','.') + '</td>
		<td>' + Cast(cast(age.Cantidad as Int) as Varchar)  + '</td>
		<td>$'+ replace(REPLACE(CONVERT(VARCHAR,CONVERT(Money, age.Total),1),'.00',''),',','.') + '</td>' + '</tr></p>'    
  ...
  /* -------------------------------------    PRESERVE THESE STYLES IN THE HEAD    ------------------------------------- */
    ...
    <body>
        <center>
            <IMG src="'+@RutaImg+'header.jpg" width="100%" HEIGHT=150>
        </center>
        <br>
     ...
     Set @HtmlCollect = '    <html>      
        ...
        <center>      
             <IMG  src="'+@RutaImg+'footer.jpg" width="100%">    
            </center>
